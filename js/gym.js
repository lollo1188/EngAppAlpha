var GymDataDownloaded;
var GymData;
var GymHeaders;
var GymOp;

function updateGymDataReceivedFromServer(GymDataDownloaded){
    // console.log("QuestDataDownloaded");
    // console.log(QuestDataDownloaded);
    GymData           = GymDataDownloaded.GymData;
...
async function importGymData(){
    console.log(GymData);
    if (!GymData) {/*se è vuoto la importo dal server! */
        console.log("we need to import gymData");

        try {
            // document.getElementById("loadingSpinner").classList.remove("d-none");
            var text = '<span> Downloading your Gym...<b> </span>'+spinnerBorder;
            var MessageId = logMessageInServerMessagesDiv(text);

            var res = await talkToServerPromise("Me","importGymData");
            ...
            updateGymDataReceivedFromServer(res);
...
function chiediRipetizioniEsercizioFatto(esercizioid){
    ...
    document.addEventListener('keypress', function enterPressed(e) {//devi dare il nome alla funzione perchè una volta completata devi rimuovere l'event lisener!!!
        ...
        if (e.key === 'Enter') {
            ...
            if (!isNaN(parseInt(inputNUMValue))){//xke 22a con parse int diventa NaN!! input è sempre una stringa però!!
